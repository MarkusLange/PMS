...
void setup() {
  ...
  // Default state after sensor power, but undefined after ESP restart e.g. by OTA flash,
  // so we have to manually wake up the sensor for sure.
  // Some logs from bootloader is sent via Serial port to the sensor after power up.
  // This can cause invalid first read or wake up so be patient and wait for next read cycle.
  pms.wakeUp();
...
void readData() {
  ...
  // Clear buffer (removes potentially old data) before read.
  // Some data could have been also sent before switching to passive mode.
  ...
  if (pms.readUntil(&data)) {
    ...
    DEBUG_OUT.println(data.pm10_standard);

    DEBUG_OUT.print("PM  2.5 (ug/m3): ");
    DEBUG_OUT.println(data.pm25_standard);

    DEBUG_OUT.print("PM 10.0 (ug/m3): ");
    DEBUG_OUT.println(data.pm100_standard);
